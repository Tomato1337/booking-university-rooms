...
Сохранение SHA-256(refresh_token) в refresh_tokens
  ↓
Response: { accessToken } + Set-Cookie: refreshToken (httpOnly, Secure, SameSite=Strict, Path=/api/auth)
```
...
Ротация: revoke текущий refresh token, генерация нового
  ↓
Response: { accessToken } + Set-Cookie: новый refreshToken
```
...
**Cookies Set:** Аналогично регистрации.
...
**Cookies Set:** Новый refresh token.
...
7. Вернуть новый access token + set новый refresh cookie
...
**Доступ:** Authenticated (Bearer token)
