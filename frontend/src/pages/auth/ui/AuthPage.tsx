...
const errorClassName =
    'mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-secondary'

const AuthBranding = reatomComponent(() => {
    const t = tAtom()
    return (
        <section
...
const RegisterForm = reatomComponent(() => {
    ...
    const { error: firstNameError, ...firstNameBind } = bindField(
        registerForm.fields.firstName,
    )
    const { error: lastNameError, ...lastNameBind } = bindField(
        registerForm.fields.lastName,
    )
    const { error: emailError, ...emailBind } = bindField(
        registerForm.fields.email,
    )
    const { error: passwordError, ...passwordBind } = bindField(
            ...
            >
                    ...
                    <div className="space-y-1.5">
                        ...
                        {firstNameError && (
                            <p className={errorClassName}>
                                {(t.validation as Record<string, string>)[
                                    firstNameError
                                ] ?? firstNameError}
                            </p>
                        )}
                ...
                <div className="space-y-1.5">
                    ...
                    <Select
                        ...
                        onValueChange={useWrap((val: string) =>
                            registerForm.fields.department.set(val),
                        )}
                    >
                        <SelectTrigger
                        ...
                        >
                            <SelectValue
                                placeholder={
                                    t.auth.register.departmentPlaceholder
                                }
                            />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    {departmentError && (
                        <p className={errorClassName}>{departmentError}</p>
                    )}
                </div>
    ...
    return (
        <main data-slot="auth-page" className="flex min-h-screen w-full">
            ...
            <section className="relative flex w-full flex-col items-center justify-center overflow-y-auto bg-surface-container-low p-8 lg:w-1/2 lg:p-24">
                ...
                <div className="absolute bottom-10 left-10 lg:hidden flex items-center gap-4">
                    <a
                        href={dashboardRoute.path()}
                        className="flex items-center gap-2 cursor-pointer"
                    >
